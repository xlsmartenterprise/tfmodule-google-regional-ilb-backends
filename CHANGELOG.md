...
### Added
- Mapped `max_utilization` in `dynamic "backend"` block for backend services using `UTILIZATION` balancing mode
- Added `lifecycle.ignore_changes` for `ip_address_selection_policy` and `locality_lb_policy` to prevent perpetual drift against GCP API defaults
